...
public class Day6 {
    ...
    static void solutionPartOne(String inputValue) {
        String temp = inputValue.replaceAll("\n", "");
        HashSet<Character> uniqueChars = new HashSet<>();

        for(int x = 0; x < temp.length(); x++) {
            uniqueChars.add(temp.charAt(x));
        }
        count = count + uniqueChars.size();
    }

    static int countForPartTwo = 0;

    static void solutionPartTwo(String inputValue) {
        String[] groupAnswers = inputValue.split("\n");
        if (groupAnswers.length == 1) {
            countForPartTwo = countForPartTwo + groupAnswers[0].length();
        } else for (int x=0; x<groupAnswers[0].length(); x++) {
            int groupIndex = 1;
            String.valueOf(groupAnswers[0].charAt(x));

            while (groupAnswers[groupIndex].contains(String.valueOf(groupAnswers[0].charAt(x)))) {
                groupIndex++;
                if (groupIndex == groupAnswers.length) {
                    countForPartTwo++;
                    break;
                }
            }
    ...
    public static void main(String[] args) {
        ...
        for (String input :
         ...
         System.out.println("Solution counter: " + countForPartTwo);
    }
}
